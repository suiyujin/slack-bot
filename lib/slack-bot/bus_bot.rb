...
class BusBot < Bot
  def create_response
    ...
    res = buses.flatten.select { |bus| bus.time > @time }.sort_by(&:time)[0...10]
    binding.pry
  end
  ...
  def scrape_timetable(bus_list)
    ...
    url = "http://transfer.navitime.biz/odakyubus/pc/diagram/BusDiagram?orvCode=#{orv_code}&course=#{course}&stopNo=#{stop_no}&date=#{@time.strftime('%Y-%m-%d')}"
    ...
    page.xpath('//div[@id="diagram-pannel"]/table/tr[@class="l2"]/th[@class="hour"]').each do |hour_list|
      ...
      buses << hour_list.next.next.xpath('div[@class="diagram-item"]').map do |minute|
        ...
        # TODO: 00時以降の考慮
        bus_time = Time.new(@time.year, @time.month, @time.day, hour, mm)
